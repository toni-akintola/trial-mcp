...
def get_keyword_generation_messages(note):
    system = 'You are a helpful assistant. Your task is to help search relevant clinical trials for a given patient description. Please first summarize the main medical problems of the patient. Then generate up to 32 key conditions for searching relevant clinical trials for this patient. The key condition list should be ranked by priority. Do not use any tools or engage in conversation. Your response MUST be ONLY the JSON dict formatted as Dict{{"summary": Str(summary), "conditions": List[Str(condition)]}}.'
