...
async def main_async():
    ...
    try:
        ...
        # Derive sample_suffix from the retrieved_nctids_file_path for output naming consistency
        base_retrieved_filename = os.path.basename(retrieved_nctids_file_path)
        # Example: retrieved_nctids_claude-3-sonnet-latest_sigir_sample2.json
        # Example: retrieved_nctids_claude-3-sonnet-latest_sigir.json

        sample_suffix_str = ""
        # Try to find "_sample<number>" at the end of the filename (before .json)
        retrieved_parts = base_retrieved_filename.replace(".json", "").split("_")
        if len(retrieved_parts) > 0 and retrieved_parts[-1].startswith("sample"):
            sample_suffix_str = "_" + retrieved_parts[-1]
